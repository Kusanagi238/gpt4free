...
# Avoid importing all provider submodules at module import time.
# Import provider implementations lazily when actually needed to prevent
# executing provider package submodules (which on some Python versions
# can trigger SyntaxError on import if they use newer syntax/features).
import importlib
...
def import_provider(name: str) -> Any:
    """Lazily import and return the provider class/variable named `name`.

    This avoids importing all providers during module import.
    Returns None if the symbol cannot be resolved.
    """
    if name in _provider_cache:
        return _provider_cache[name]

    # Try to import a submodule matching the provider name (lowercased),
    # then fall back to the provider package attribute if available.
    try:
        mod = importlib.import_module(f".Provider.{name.lower()}", package=__package__)
        sym = getattr(mod, name, None)
    except Exception:
        try:
            pkg = importlib.import_module(".Provider", package=__package__)
            sym = getattr(pkg, name, None)
        except Exception:
            sym = None

    _provider_cache[name] = sym
    return sym


# Example usage elsewhere in the module:
# MyProvider = import_provider("Qwen")
# if MyProvider is not None:
#     instance = MyProvider(...)
